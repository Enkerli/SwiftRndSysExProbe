#!/usr/bin/env python3
"""Checks SwiftRndSysExProbe's audio component identity.
...
`Pqf1`, which belongs to the JUCE SwiftRndSysExProbe and would collide with the AUv3 that
...
Note that both are called "SwiftRndSysExProbe". Sharing a display name is legal — the
triple is the identity and hosts route on it — but a host lists them by name, so
two entries reading "Enkerli: SwiftRndSysExProbe" is a thing to decide about rather than
to discover.
...
Checks:
  ...
  4. The display name follows the "Manufacturer: Product" convention hosts
     expect, so AUM shows "SwiftRndSysExProbe" rather than a target name.
...
INFO_PLIST = REPO / "SwiftRndSysExProbeExtension" / "Info.plist"
PROJECT = REPO / "SwiftRndSysExProbe.xcodeproj" / "project.pbxproj"
HOST_MODEL = REPO / "SwiftRndSysExProbe" / "Model" / "AudioUnitHostModel.swift"
...
def main() -> None:
    ...
    # in AUM at all**. Same story waiting for RndSysExProbe, whose JUCE CMakeLists
    # sets `com.enkerli.RndSysExProbe` and carries its own warning: "installed
    # devices have it".
    #
    ...
    # CLAUDE.md — SwiftRndSysExProbe and SwiftDrawnQurve both said `aumi/Srpe/Enke`,
